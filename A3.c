...
{
	// Atualizar a posição vertical dos tiros
	update_shots(board, list);

	// Verificar e criar novos tiros
	for (int y = 0; y < board->max_y; y++)
	{
		for (int x = 0; x < board->max_x; x++)
		{
			if (board->map[x][y].entity != NULL)
			{
				// Verificar se é possível criar um novo tiro para este inimigo
...
int main(int argc, char **argv)
{
	...
	do
	{
		...
		case 'x':
			...
			{
				fprintf(stderr, "ERRO: o número de colunas é inválido !\n");
				return 2;
			}
			break;
		case 'y':
		...
		case 'e':
			...
			{
				fprintf(stderr, "ERRO: o número de linhas de inimigos é inválido!\n");
				return 4;
			}
			break;
		case 'r':
			...
			{
				fprintf(stderr, "ERRO: o número de rodadas é inválido!\n");
				return 5;
			}
			break;
		case 'o':
